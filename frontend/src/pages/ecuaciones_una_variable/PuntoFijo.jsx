...
const PuntoFijo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!funcion.trim() || !p0.trim()) {
      setError('Por favor, completa la función y p0.');
      setResultado(null);
      return;
    }

    try {
      const res = await axios.post('http://localhost:5000/ecuacion/punto-fijo', {
        funcion,
        p0,
        ...(TOL && { TOL }),
        ...(N0 && { N0 }),
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="text-gray-700 space-y-5 text-base">
            <div className="space-y-3">
                <p>
                    El <strong>método de iteración de punto fijo</strong> es un método numérico para encontrar una solución a la ecuación 
                    <code className="bg-gray-200 px-1 rounded">p = g(p)</code>. La idea es partir de una aproximación inicial (<code className="bg-gray-200 px-1 rounded">p0</code>) y generar una sucesión 
                    definida por <code className="bg-gray-200 px-1 rounded">p(i+1) = g(pi)</code>.                    
                </p>
