...
const PuntoMedioCompuesta = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!funcion.trim() || !a.trim() || !b.trim() || !n.trim()) {
    ...
    try {
      const res = await axios.post('http://localhost:5000/calculo/regla_punto_medio_compuesta', {
        funcion,
        a,
        b,
        n,
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="text-gray-700 space-y-5 text-base">
          <div className="space-y-3">
            <p>
                La <strong>regla del punto medio compuesta</strong> aproxima la integral de una función <code className="bg-gray-200 px-1 rounded">f(x)</code> usando el 
                valor de la función en el punto medio de cada subintervalo. Divide el intervalo <code className="bg-gray-200 px-1 rounded">[a,b]</code> en  
                <code className="bg-gray-200 px-1 rounded">n</code> subintervalos y evalúa <code className="bg-gray-200 px-1 rounded">f</code>
                en el centro de cada par de intervalos.
          </p>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md space-y-6">
          <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1">
                <label className="block mb-1 font-semibold">n (subintervalos):</label>
                <input
                  type="text"
                  value={n}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (/[A-Z]/.test(value)) {
                      setNError('No se permiten mayúsculas.');
                    } else {
                      setNError('');
                      setN(value);
                    }
                  ...
                  required
                />
                {nError && <p className="text-red-500 text-sm mt-1">{nError}</p>}
              </div>
            </div>
          </div>
        ...
        {resultado && resultado !== 'calculando' && (
          <div className="bg-green-100 border border-green-400 text-green-700 p-6 rounded-lg space-y-2">
            ...
            <p><strong>Valor de la integral:</strong> {resultado.resultado}</p>
          </div>
        )}
...
export default PuntoMedioCompuesta;
