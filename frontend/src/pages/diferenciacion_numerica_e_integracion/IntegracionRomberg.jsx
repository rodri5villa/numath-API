...
const IntegracionRomberg = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/calculo/integracion_romberg', {
        funcion,
        a,
        b,
        n
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="text-gray-700 space-y-5 text-base">
          <div className="space-y-3">
            <p>
                El <strong>método de integración de Romberg</strong> aproxima la integral de una función <code className="bg-gray-200 px-1 rounded">f(x)</code> 
                en <code className="bg-gray-200 px-1 rounded">[a,b]</code> que combina la regla del trapecio 
                compuesta con extrapolación de Richardson para acelerar la convergencia. 
            </p>
        ...
        {resultado && resultado !== 'calculando' && (
          <div className="bg-green-100 border border-green-400 text-green-700 p-6 rounded-lg space-y-4">
            <h2 className="text-xl font-semibold">Resultado:</h2>
            <ul className="list-disc ml-6 space-y-1">
              {resultado.resultado.map((fila, i) => (
                <li key={i}>
                  <code className= "px-2 py-1 rounded inline-block">{fila}</code>
                </li>
              ))}
            </ul>
          </div>
...
export default IntegracionRomberg;
